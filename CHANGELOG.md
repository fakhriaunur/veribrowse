...
- Final README + micro-cleanup (no app behavior change): `README.md` rewritten to the final state (Next 16.3.4 Turbopack + clean-`.next` note, React 19.2.8, Node 22.23.2, pnpm-only single lockfile via `packageManager` + Corepack, openai 7 / zod 4 / pino 10, toolchain majors, live URL `https://veribrowse.netlify.app`, setup/run/test/replay/smoke, env vars, solo-maintainer branch policy — every command verified by running it). `app/globals.css`: deleted the unused `@theme --color-safe/caution/risky` + `:root` tokens (TrustBadge keeps hardcoded Tailwind classes, zero visual change). `infra/netlify.toml` header synced to the root Next.js 16 header (comment-only). Stale pins removed from `docs/` + `AGENTS.md` + route comment (history stays in this CHANGELOG only).
